  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Puter FS Subdirectory Write Error, falling back to root:", fsError);
      ...
      JSON SCHEMA:
      {
        ...
        "feedback": {
          "toneAndStyle": { "score": number, "description": "1-2 sentence assessment of language and impact" },
          "content": { "score": number, "description": "1-2 sentence assessment of relevance and depth" },
          "structure": { "score": number, "description": "1-2 sentence assessment of layout and readability" },
          "skills": { "score": number, "description": "1-2 sentence assessment of technical stack alignment" }
        },
      ...
      try {
        ...
        response = await window.puter.ai.chat(`${systemPrompt}\n\n${userPrompt}`, { 
          model: 'gpt-4o'
        });
      }
      ...
      let jsonStr = markdownMatch ? markdownMatch[1] : (curlyMatch ? curlyMatch[1] : content);
      ...
      try {
        ...
        throw new Error("The AI failed to format the analysis as JSON. Please refine your inputs or try again.");
      }
      ...
      console.error("Full Processing Error Output:", error);
      ...
      if (error instanceof Error) {
        ...
        errorMessage = error?.message || error?.error?.message || JSON.stringify(error) || "Operation failed";
      }
      ...
      <div className="min-h-screen bg-slate-50 flex flex-col">
        ...
        <main className="flex-1 flex items-center justify-center p-8">
          <div className="flex flex-col items-center gap-4 text-center">
            ...
            <p className="text-slate-400 font-medium animate-pulse">Establishing Audit Environment...</p>
          </div>
        </main>
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <main className="flex-1 max-w-3xl w-full mx-auto px-4 py-16">
        <div className="mb-12">
          ...
          <p className="text-slate-500 mt-2 font-medium">Get a deep technical analysis of your resume alignment.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-1">Job Description Fragment</label>
            <textarea 
              required
              rows={5}
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-1">Source Resume (PDF)</label>
            {!file ? (
              <div 
                {...getRootProps()} 
                className={`border-2 border-dashed rounded-2xl p-16 flex flex-col items-center justify-center cursor-pointer transition-all ${
                  isDragActive ? 'border-indigo-500 bg-indigo-500/5' : 'border-slate-200 bg-white hover:bg-slate-50'
                }`}
              >
                <input {...getInputProps()} />
                ...
                <p className="text-sm font-bold text-slate-800">Click or drag a PDF here</p>
                <p className="text-xs text-slate-400 mt-2 font-medium uppercase tracking-tighter">Maximum file size: 5MB</p>
              </div>
            ) : (
              <motion.div 
          ...
          >
            ...
            <div className="max-w-md space-y-4">
              ...
              <p className="text-slate-500 font-medium leading-relaxed">
                Our AI auditor is cross-referencing your experience with industry benchmarks for {jobTitle} roles.
              </p>
