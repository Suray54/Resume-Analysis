    ...
    if (resume?.pdfPath && window.puter) {
      ...
      if (newTab) {
        ...
        newTab.document.body.innerHTML =
          '<div style="display:flex;justify-content:center;align-items:center;height:100vh;font-family:sans-serif;color:#666;">Loading your resume...</div>';
      }
      ...
      try {
        ...
        if (newTab) {
          newTab.document.body.innerHTML =
            '<div style="display:flex;justify-content:center;align-items:center;height:100vh;font-family:sans-serif;color:#e11d48;">Failed to load PDF. Please try again.</div>';
        }
      }
      ...
      if (newTab) {
        newTab.document.write(
          `<img src="${resume.previewUrl}" style="max-width: 100%; height: auto; display: block; margin: 0 auto;">`,
        );
  ...
  if (!resume) {
    return (
      <div className="min-h-screen bg-neutral-50 flex flex-col items-center justify-center p-4">
        ...
        <Link
          href="/"
          className="mt-8 bg-neutral-900 text-white px-8 py-3 rounded-full font-bold"
        >
          Go Back Home
        </Link>
      </div>
        ...
        <header className="h-16 border-b border-slate-200 bg-white px-8 flex items-center justify-between shadow-sm z-10 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-slate-500 font-medium max-w-[400px] truncate hidden sm:block">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                  ...
                  <div className="p-6 rounded-2xl bg-white border border-slate-100 shadow-sm">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {resume.matchedKeywords?.map((kw, i) => (
                        <span
                          key={i}
                          className="px-3 py-1 bg-emerald-50 text-emerald-700 rounded-full text-[11px] font-bold border border-emerald-100"
                        >
                          {kw}
                        </span>
                      ))}
                  ...
                  <div className="p-6 rounded-2xl bg-white border border-slate-100 shadow-sm">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {resume.missingKeywords?.map((kw, i) => (
                        <span
                          key={i}
                          className="px-3 py-1 bg-rose-50 text-rose-700 rounded-full text-[11px] font-bold border border-rose-100"
                        >
                          {kw}
                        </span>
                      ))}
              ...
              <div className="lg:col-span-8 space-y-6">
                    ...
                    <ul className="space-y-4">
                      {resume.strengths && resume.strengths.length > 0 ? (
                        resume.strengths.map((item, i) => (
                          <li
                            key={i}
                            className="flex gap-3 text-slate-600 text-sm leading-relaxed font-medium"
                          >
                ...
                {[
                  ...
                  <div
                    key={section.id}
                    className="p-8 rounded-2xl bg-white border border-slate-100 shadow-sm transition-all hover:shadow-md overflow-hidden"
                  >
                    <button
